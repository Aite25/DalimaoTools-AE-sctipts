    ...
    function buildUI(container) {
        ...
        "group { orientation:'column', alignment:['left','top'], alignChildren:['center','center'], \
            ...
            gr5: Group { orientation:'row', alignment:['fill','fill'], \
                lineSweepBtn: Button { text:'CC Line Sweep',alignment:['left','top'], preferredSize:[280,20] }    \
            }, \
        }"; 
        ...
        // CC Line Sweep 按钮
        gr.gr5.lineSweepBtn.onClick = function () {
            app.beginUndoGroup("Add CC Line Sweep");
            var selectedLayers = app.project.activeItem.selectedLayers;
            var curComp = app.project.activeItem;
            var timeDelta = frames * curComp.frameDuration;
            
            for(var i = 0; i < selectedLayers.length; i++) {
                // 添加 CC Line Sweep 效果
                var lineSweep = selectedLayers[i].Effects.addProperty("CC Line Sweep");
                
                // 设置 Direction 为角度值
                lineSweep.property("Direction").setValue(angle);
                
                // 设置 Completion 关键帧：从 100 到 0
                var completionProp = lineSweep.property("Completion");
                completionProp.setValueAtTime(curComp.time, 100);
                completionProp.setValueAtTime(curComp.time + timeDelta, 0);
            }
...
if ($.global.DALIMAO_LOADER_ACTIVE) {
    $.global.DALIMAO_REGISTER(PositionOffsetModule);
} else {
    (function() {
